...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      ...
      //
      // @rolldown/browser is intentionally bundled here (not CDN): its WASI
      // worker does `new Worker(new URL('./wasi-worker-browser.mjs', import.meta.url))`,
      // which throws a cross-origin SecurityError when loaded from esm.sh. Vite
      // emits it as a same-origin worker chunk under /demo/assets/ (issue #12).
      external: [
