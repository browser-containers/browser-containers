...
describe('createModuleShim', () => {
  it('resolves node builtins via the supplied getShim', () => {
    ...
    const pathShim = { sep: '/' };
    const moduleShim = createModuleShim({ vfs, getShim: (name) => (name === 'path' ? pathShim : undefined) });
  ...
  it('reads JSON files synchronously off the vfs, resolved relative to the requiring file', () => {
    const vfs = new VfsBus();
    (vfs.hot as unknown as { mkdirSync: (p: string, o?: unknown) => void }).mkdirSync('/pkg', { recursive: true });
