...
export const createModuleShim = (options: ModuleShimOptions) => {
  ...
  const createRequire = (filename: string) => {
    const require = (specifier: string): unknown => {
      ...
      if (BUILTIN_MODULE_NAMES.includes(bare)) {
        ...
        if (shim) return shim;
        throw new Error(`require("${specifier}") has no browser shim registered for node builtin "${bare}".`);
      }

      if (specifier.endsWith('.json') && (specifier.startsWith('.') || specifier.startsWith('/'))) {
        const resolved = specifier.startsWith('/') ? specifier : joinPath(dirname(filename), specifier);
    ...
    require.resolve = Object.assign(
      (specifier: string): string => {
        ...
        throw new Error(`require.resolve("${specifier}") is not supported for npm packages post-bundle.`);
      },
  ...
  return {
    ...
    isBuiltin: (id: string): boolean =>
      id.startsWith('node:') || BUILTIN_MODULE_NAMES.includes(id) || UNSUPPORTED_BUILTIN_NAMES.includes(id),
  };
};
