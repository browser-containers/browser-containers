...
describe('child_process shim', () => {
  ...
  it('spawn exposes stdout/stderr as real streams emitting the dispatched output (A6)', async () => {
    const registry = {
      dispatch: async (_cmd: string, _args: string[]) => ({ stdout: 'out-line', stderr: 'err-line', exitCode: 0 }),
    };
    const shim = createChildProcessShim(registry);
  ...
  it('exec callback receives the real dispatched stdout/stderr (A6)', async () => {
    const registry = {
      dispatch: async (_cmd: string, _args: string[]) => ({ stdout: 'hello\n', stderr: '', exitCode: 0 }),
    };
    const shim = createChildProcessShim(registry);

    const result = await new Promise<{ error: Error | null; stdout: string; stderr: string }>((resolve) => {
      shim.exec('echo hello', (error: Error | null, stdout: string, stderr: string) => resolve({ error, stdout, stderr }));
    });
  ...
  it('exec callback receives an error on non-zero exit (A6)', async () => {
    const registry = {
      dispatch: async (_cmd: string, _args: string[]) => ({ stdout: '', stderr: 'boom', exitCode: 1 }),
    };
    const shim = createChildProcessShim(registry);
